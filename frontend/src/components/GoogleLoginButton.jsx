import { useState, lazy, Suspense } from 'react';
import { motion } from 'framer-motion';

// Check if Google Client ID is configured
const isGoogleConfigured = Boolean(import.meta.env.VITE_GOOGLE_CLIENT_ID);

// Google Icon Component (colored version)
const GoogleIconColored = () => (
  <svg className="w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      fill="#4285F4"
    />
    <path
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      fill="#34A853"
    />
    <path
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      fill="#FBBC05"
    />
    <path
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      fill="#EA4335"
    />
  </svg>
);

// Google Icon Component (grayed version for disabled state)
const GoogleIconGrayed = () => (
  <svg className="w-5 h-5 opacity-50" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#9CA3AF"/>
    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#9CA3AF"/>
    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#9CA3AF"/>
    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#9CA3AF"/>
  </svg>
);

// Disabled button when Google is not configured
const DisabledGoogleButton = () => (
  <button
    type="button"
    disabled
    className="w-full flex items-center justify-center gap-3 px-4 py-2 rounded-lg font-medium bg-gray-100 dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-800 text-gray-400 dark:text-gray-600 cursor-not-allowed"
    title="Google login not configured. Add VITE_GOOGLE_CLIENT_ID to your .env file."
  >
    <GoogleIconGrayed />
    <span>Google login not configured</span>
  </button>
);

// Lazy loaded active button component
const ActiveGoogleButtonComponent = lazy(() => import('./GoogleLoginButtonActive'));

// Main exported component that conditionally renders based on configuration
const GoogleLoginButton = ({ onSuccess, onError, disabled = false }) => {
  if (!isGoogleConfigured) {
    return <DisabledGoogleButton />;
  }
  
  return (
    <Suspense fallback={
      <button
        type="button"
        disabled
        className="w-full flex items-center justify-center gap-3 px-4 py-2 rounded-lg font-medium bg-white dark:bg-black border-2 border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300"
      >
        <div className="w-5 h-5 border-2 border-gray-300 border-t-gray-600 dark:border-gray-600 dark:border-t-gray-300 rounded-full animate-spin" />
        <span>Loading...</span>
      </button>
    }>
      <ActiveGoogleButtonComponent 
        onSuccess={onSuccess} 
        onError={onError} 
        disabled={disabled} 
      />
    </Suspense>
  );
};

export default GoogleLoginButton;
